...
export async function fetchKlines(
  ...
  try {
    ...
    console.error(`Error fetching klines for ${exchange} ${symbol}:`, err);
    return [];
  }
}
...
export async function runScreenerScan(params: {
  ...
  // Scan candidates (up to 120 coins to scan all active coins swiftly)
  const selectedTickers = uniqueTickers.slice(0, 120);

...
export const TerminalPage: React.FC<TerminalPageProps> = ({
  ...
  const computedHeightStyle = useMemo(() => {
    ...
    if (!config.autoFitScreen) {
      switch (config.blockHeight) {
        case 'compact': return '380px';
        case 'medium': return '520px';
        case 'large': return '680px';
  ...
  const isPositive = (activeCoin.priceChange24h || 0) >= 0;
    ...
    >
      ...
      <header className="h-12 sm:h-14 bg-slate-950/95 border-b border-slate-800 px-2.5 sm:px-4 flex items-center justify-between gap-2 shrink-0 z-20 overflow-x-auto no-scrollbar touch-scroll">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          >
            <span>${formatCryptoPrice(activeCoin.currentPrice || 0)}</span>
            <span className="text-[10px] hidden sm:inline">
              ({isPositive ? '+' : ''}{(activeCoin.priceChange24h || 0).toFixed(2)}%)
            </span>
          </div>
        </div>
      ...
      <main className="flex-1 w-full h-full relative overflow-hidden bg-[#090d16]">
        ...
        {blocks.length <= 1 ? (
            ...
            <div className="w-full h-full relative overflow-hidden">
              ...
              <div className="absolute bottom-2 left-2 z-20 flex items-center gap-2">
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-md bg-transparent text-slate-300 transition-colors hover:bg-slate-800/60 hover:text-cyan-300 cursor-pointer"
          ...
          <div className="w-full h-full p-2 sm:p-3 overflow-y-auto">

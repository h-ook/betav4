...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup') {
      ...
      if (!cleanCode) {
        setValidationError('Введіть спеціальний код доступу з Telegram каналу автора');
        return;
      }
    }
    ...
    try {
      if (mode === 'signin') {
        ...
        await registerWithEmail(cleanEmail, password, displayName, inviteCode);
      }
      onSuccess?.();
      onClose();
    } catch {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-200 overflow-y-auto ${
        isRestrictedMode ? 'cursor-default' : ''
      }`}
      onClick={(e) => {
        // Prevent closing on backdrop if in restricted mode
        if (e.target === e.currentTarget && !isRestrictedMode) {
          onClose();
        }
      }}
    >
      ...
      >
        {/* Security / Restricted Notice Banner */}
        {isRestrictedMode && (
          <div className="bg-gradient-to-r from-amber-500/20 via-cyan-500/20 to-indigo-500/20 px-4 py-2 border-b border-amber-500/30 flex items-center justify-between text-[11px] text-amber-200">
            <div className="flex items-center gap-1.5 font-semibold">
              <ShieldAlert className="w-3.5 h-3.5 text-amber-400 shrink-0" />
              <span>Доступ тільки для зареєстрованих трейдерів</span>
            </div>
            <span className="text-[10px] px-2 py-0.5 rounded-full bg-amber-500/20 border border-amber-500/30 font-mono font-bold text-amber-300">
              Закритий скринер
            </span>
          </div>
        )}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-white tracking-wide">
                {mode === 'signin' ? 'Вхід у скрінер' : 'Реєстрація за кодом доступу'}
              </h2>
              <p className="text-[11px] text-slate-400">
                {mode === 'signin'
                  ? 'Особисті сповіщення, MetaScalp та обране'
                  : 'Без підтвердження пошти — миттєвий вхід'}
              </p>
            </div>
          </div>
          {!isRestrictedMode && (

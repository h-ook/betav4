      ...
      <div className="flex items-center justify-between gap-1.5 px-2.5 py-1.5 bg-slate-950 border-b border-slate-800/90 text-xs shrink-0 select-none">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <div className="flex items-center gap-1 ml-1 font-mono">
            <span className="text-white font-bold text-xs">${formatCryptoPrice(livePrice)}</span>
            <span
              className={`text-[10px] font-semibold hidden xs:inline ${
          ...
          {/* Active Formation tag if present */}
          {activeFormation && (
            <span
              className="text-[10px] px-1.5 py-0.5 rounded bg-purple-950/80 text-purple-300 border border-purple-800/50 flex items-center gap-1 font-semibold hidden md:flex"
              title={`${activeFormation.name} (${activeFormation.bias})`}
            >
              <Layers className="w-2.5 h-2.5 text-purple-400" />
              <span className="truncate max-w-[90px]">{activeFormation.name}</span>
            </span>
          )}
        </div>

        {/* Right: Timeframe pills, mode toggle, MetaScalp, Telegram, Maximize, Close */}
        <div className="flex items-center gap-1 shrink-0">
          {/* Timeframe selector */}
          <div className="flex items-center bg-slate-900 p-0.5 rounded-lg border border-slate-800/80 font-mono text-[10px]">
            {(['1m', '5m', '15m', '1h', '4h', '1d'] as Timeframe[]).map((tf) => (
              <button
                key={tf}
                onClick={() => onUpdateBlock({ timeframe: tf })}
        ...
        {block.mode === 'tradingview' ? (
          ...
          <div className="w-full h-full absolute inset-0">
            <TradingViewChart
              ...
              fullHeight={true}
              onTimeframeChange={(newTf) => onUpdateBlock({ timeframe: newTf })}
            />
          </div>
        )}
      </div>
